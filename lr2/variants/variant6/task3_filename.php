...
function extractDirectory(string $path): string
{
    ...
    return str_contains($path, '\\') ? str_replace('/', '\\', $dir) : $dir;
}

$path = $_POST['path'] ?? 'F:\Archive\Docs\contract.pdf';
...
<div class="demo-card">
    ...
    <p class="demo-subtitle">Розбір повного шляху на директорію, ім'я та розширення</p>

    <form method="post" class="demo-form">
        <div>
            <label for="path">Повний шлях до файлу</label>
            <input type="text" id="path" name="path" value="<?= htmlspecialchars($path) ?>" placeholder="C:\Folder\file.txt">
        </div>
    ...
    <div class="demo-code">
// Результат для: "<?= htmlspecialchars($path) ?>"
// Директорія: "<?= htmlspecialchars($directory) ?>"
// Імʼя файлу: "<?= htmlspecialchars($filename) ?>"
// Розширення: "<?= htmlspecialchars($extension) ?>"
    </div>
</div>
<?php
